...
\item [Couplingfile] [File name] (The file contains one Hamiltonian snapshot from which the couplings will be used.)
\item [Positionfile] [File name] (Only needed for TransitionDipole or ExtendedDipole HamiltonianType or for exciton diffusion calculations)
\item [SingeShiftfile] [File name] (File containing site specific energy shifts.
The first number in the file is the number of sites to shift followed by lines with site index (starting from 0),
scaling factor, $a$, and shift, $b$, in cm$^{-1}$. The new frequency is calculated as $\omega_{{new}}=\omega_{{old}}*a+b$.  This file is only used if the keyword SingeShiftfile is included in the input.)
