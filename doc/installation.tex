...
{\small Work by Thomas Jansen, Carleen D. N. van Hengel, Hoang Long Nguyen, Kai Zhong, Vesna Eric, Gijsbert ten Hoven,
Stephanie Gonzalez Migoni, Marick Manrho, Ana Cunha, Adriana Armaczki, and Kim van Adrichem}
\begin{itemize}
\item Added Runge Kutta 4 propagation scheme (Thomas Jansen)
\item Added fluorescence detected 2D spectroscopy with coarse grained NISE (Stephanie Gonzalez Migoni)
\item Added the calculation of radiatice rates from luminescence spectra (Adriana Armaczki)
\item Added the calculation of Redfield transfer matrices (Thomas Jansen)
\item Added calculation of spectral densities, correlation functions and lineshape functions (Thomas Jansen)
...
\item The 2DFFT code was cleaned up (Thomas Jansen)
\item Raman and 2DIRraman techniques were added (Carleen D. N. van Hengel)
\item openMP parallel CD and DOS were implemented (Thomas Jansen)
\item Diffusion calculations were added (Thomas Jansen)
\item On the fly transition-dipole and extended-dipole coupling schemes were impemented (Thomas Jansen)
\item Added project file option to project on substructures - including multi segment options for linear techniques (Thomas Jansen)
\item Added inhomogeneous and homogeneous apodization functions (Thomas Jansen)
\item Added automatic check on Singles keyword providing warning to user if it may be incorrect (Thomas Jansen)
