...
The linear dichroism is calculated identically to the linear absorption except the average of
the absorption in the x and y directions is subtracted from the absorption in the z direction.
...
The Boltzmann term containg the Hamiltonian at time zero ($H$) and the temperature (to be specified in the input)
ensure the emission from a termalized population of the excited state ignoring a potential Stoke's shift and effects
of vibronic states. The spectrum is normalized with the partition function ($Z$). 
The \ilk{ImaginaryTimesteps} keyword can be used to speed up the calculation of the Boltzmann term by calculating the
response function using imaginary time step propagation, which is equivalent to calculating the partition function
and the thermal population of the excited states. If the keyword is not specified the Boltzmann term is calculated
using the matrix exponential, which is computationally more expensive, but more accurate and more stable.

The radiative rate is computed based on the luminescence spectra. The spontaneous
emission rate, $k^{R}$, is obtained by integrating over the luminescence spectrum and
converting to the units of $ns^{-1}$ applying the following relation:
\begin{equation}
k_R = \frac{8\pi^2 }{3\epsilon_0 c^3 \hbar} \int \nu^3 I(\nu) d\nu
\end{equation}
For this calculation the transition-dipoles have to be provided in units of Debye. The calculation does not account
for local field effects and the prefeactor of the index of refraction ($n$), which in typical solvents environments
can increase the radiative rate by a factor of 3-5.

The multi-segment option cannot be used with this calculation yet.
