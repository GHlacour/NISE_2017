...
\section{Raman}
The Raman response is calculared as 
\begin{equation}
        I(t)=\sum_{a}^{xyz}\langle\alpha_{a}(t)U(t,0)\alpha_{a}(0)\rangle\exp(-t/T_1).
\end{equation}
Both the all parallel and perpendicular components are calculated. The calculation require that the transition-polarizability file is provided with all six independent components stored. The data are stored in the files Raman\_VV.dat and Raman\_VH.dat.
...
\section{2DIRRaman$^{*}$}
This calculates the two-dimensional infrared raman spectra assuming coupled three level systems. The techniques 2DIRraman1 (rephasing), 2DIRraman2 (non-rephasign 1), 2DIRraman3 (non-repasing 2) provides these contributions separetely. The rephaqsing diagram and the non-rephasing diagrams are stored separately as they cannot be directly added. % The expressions for the response functions are given in ref. \cite{}.
